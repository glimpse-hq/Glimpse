...
/**
 * Format a duration in seconds into a compact human-readable string (hours, minutes, or seconds).
 *
 * @param seconds - Duration in seconds
 * @returns A formatted string using:
 *  - hours with no decimals if >= 100 hours (e.g. "120h"),
 *  - hours with one decimal if >= 10 hours (e.g. "12.3h"),
 *  - hours with up to two decimals trimmed of trailing zeros if >= 1 hour (e.g. "1.5h" or "1h"),
 *  - minutes rounded to whole minutes if >= 1 minute (e.g. "45m"),
 *  - seconds rounded to the nearest whole second otherwise (e.g. "30s")
 */
...
/**
 * Format a timestamp into a short, localized date string or "Unknown" when no timestamp is provided.
 *
 * @param timestamp - Unix timestamp in milliseconds (or `null`/`undefined`). A falsy value (including `0`) will be treated as absent.
 * @returns `"Unknown"` if `timestamp` is falsy; otherwise a localized short date string (e.g., "Jan 2, 2024").
 */
...
/**
 * Selects an appropriate device or OS icon component based on the provided OS and client names.
 *
 * @param osName - Operating system name or identifier (matched case-insensitively)
 * @param clientName - Client or application name (used to detect phone clients)
 * @returns A JSX element rendering an icon for the detected OS/device: Apple, Windows, Linux, Smartphone, or a generic Monitor as fallback
 */
...
/**
 * Builds a human-readable description for an authentication session.
 *
 * @param session - The session whose details should be formatted; may include `osName`, `osVersion`, `appVersion`, and `createdAt`.
 * @returns A single string containing the OS and version (when available), the app version prefixed with "Glimpse" (when available), and a "Signed in <date>" timestamp, with parts joined by " • ".
 */
...
/**
 * Initiates a checkout session for the specified pack and opens the returned URL.
 *
 * @param packId - Identifier of the pack to purchase (`"starter"`, `"standard"`, or `"power"`)
 * @throws If the checkout action does not return a URL
 */
...
const AccountView = ({
  ...
  /**
   * Loads cloud usage statistics for the current user and updates component state.
   *
   * On success, replaces the current usageStats with the fetched values. On failure, logs the error and leaves usageStats unchanged.
   *
   * @param showLoading - If true, toggles the usage-stats loading indicator while the fetch is in progress (default: `true`).
   */
  ...
  /**
   * Loads the current user's active authentication sessions into component state.
   *
   * Attempts to fetch active sessions, replaces the local sessions list with the result,
   * and toggles the loading indicator while the operation is in progress. Errors are
   * logged to the console and the loading indicator is cleared in all cases.
   */
  ...
  /**
   * Persist the edited display name when it has changed.
   *
   * Trims the current edit and, if the result is non-empty and different from the existing name,
   * updates the user's name on the server, persists the change locally, invokes the parent update
   * callback, and updates local loading and editing state. Errors are logged to the console.
   */
  ...
  /**
   * Revokes the session with the given ID and removes it from the local sessions list.
   *
   * @param sessionId - The ID of the session to revoke; triggers a deletion request and updates component state to remove that session.
   */
  ...
  /**
   * Signs the user out of all non-current sessions and refreshes the session list.
   *
   * Sets the local signing-out state while the operation runs; on success it reloads the sessions.
   * If an error occurs it is logged to the console and the signing-out state is cleared.
   */
